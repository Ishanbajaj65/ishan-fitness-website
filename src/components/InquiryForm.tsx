...
export default function InquiryForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Direct post request to Formspree servers—completely bypassing Supabase client bugs
      const response = await fetch('https://formspree.io/f/mvzyqovr', {
        method: 'POST',
        headers: {
    ...
    return (
      <div className="bg-zinc-950 border border-zinc-900 rounded-3xl p-8 text-center space-y-4 max-w-md mx-auto my-12">
        ...
        <p className="text-xs text-zinc-400 leading-relaxed">
          Your parameters have been logged. Ishan will review your information and reach out on WhatsApp shortly.
        </p>
        <button onClick={() => setIsSuccess(false)} className="text-xs font-mono text-lime-400 hover:underline pt-2 block mx-auto">
